...
void *GenerateGuesses(void *arg) {
    ...
    for (int i = data->start; i < data->end; i++) {
        string guess = data->prefix + data->a->ordered_values[i];
...
void PriorityQueue::Generate(PT pt) {
    ...
    if (pt.content.size() == 1) {
        ...
        int chunk_size = static_cast<int>(a->ordered_values.size()) / num_threads;
        for (int i = 0; i < num_threads; i++) {
            thread_data[i] = {
                a,
                i * chunk_size,
                (i == num_threads - 1) ? static_cast<int>(a->ordered_values.size()) : (i + 1) * chunk_size,
                &guesses,
                &mutex,
                &total_guesses,
                ""
            };
            pthread_create(&threads[i], nullptr, GenerateGuesses, &thread_data[i]);
        }
